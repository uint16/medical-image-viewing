...
public class CoronalReconstructionStrategy implements DisplayStrategy, Serializable {
	...
	public JPanel getPanel(int index, Study s) {
		...
		BufferedImage recon = new BufferedImage(s.getImage(index).getWidth(), s.imgAmt(), s.getImage(index).getType());
		Graphics2D rg = recon.createGraphics();
		for(int i = 0; i < s.imgAmt(); i++){
			MedicalImage tmpImg = s.getImage(i);
			rg.drawImage(tmpImg.getSubImage(0, reconstructionIndex, tmpImg.getWidth(), 1), 0, s.imgAmt()-i, null);
		}
